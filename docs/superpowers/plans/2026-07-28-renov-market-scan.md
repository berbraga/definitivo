...
def test_installment_with_connectors_between_count_and_amount():
    """'em ate 12x sem juros de R$ X' is the most common phrasing on Mercado Livre."""
    for title in (
        "iPhone 13 128GB em ate 12x sem juros de R$ 254,17",
        "iPhone 13 128GB em até 12x sem juros de R$ 254,17",
    ):
        price, reason = extract_price(title)
        assert price is None
        assert reason == PRICE_INSTALLMENT
...
def test_installment_with_cartao_variants():
    ...
    for title in (
        "iPhone 13 128GB 10x no cartão de R$ 199,00",
        "iPhone 13 128GB 10x no cartao de R$ 199,00",
    ):
        price, reason = extract_price(title)
        assert price is None
        assert reason == PRICE_INSTALLMENT
...
Os seis ultimos testes existem porque a versao original destas regex tinha tres furos,
todos com contraexemplo real: (1) o prefixo exigia `Nx` colado ao valor, entao
`"em ate 12x sem juros de R$ 254,17"` devolvia a PARCELA como preco a vista — furo na
unica regra inviolavel do modulo; (2) `cart[ao][eo]?` nunca casava `cartão`, porque
este modulo le texto cru e o `ã` esta fora da classe ASCII; (3) qualquer `Nx` antes do
valor era lido como contagem de parcela, entao `"Space Zoom 100x R$ 4.500,00"` era
descartado, e a regra de sufixo condenava `"R$ 3.050,00 sem juros no cartao"` mesmo sem
nenhuma contagem no texto. Dai a contagem limitada a dois digitos, os conectores
acentuados escritos e a remocao da regra de sufixo.
